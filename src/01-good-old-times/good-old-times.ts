...
export function main() {
    ...
    const stopWordFd = fs.openSync(path.join(__dirname, "../stop_words.txt"), "r");
    ...
    const wordFreqs = touchOpen(path.join(__dirname, "result/word_freqs.txt"), "rb+");

    const inputFile = fs.readFileSync(path.join(__dirname, "../pride-and-prejudice.txt"), "utf-8").split("\n");
            ...
            if (data[2] === null) {
                ...
                if (!c.match(/[a-zA-Z0-9]/)) {
                    ...
                    if (data[5].length >= 2 && !data[0].includes(data[5])) {
                        ...
                        if (!data[4]) {
                            fs.writeSync(wordFreqs, data[5] + "," + ("0000" + 1).slice(-4) + "\n");
                        } else {
                            fs.writeSync(wordFreqs, data[5] + "," + ("0000" + data[7]).slice(-4) + "\n");
                        }
                    }
                    data[2] = null;
                }
